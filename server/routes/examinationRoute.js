...
router.post("/", async (req, res) => {
  ...
  try {
    ...
    if (!mongoose.Types.ObjectId.isValid(sessionId)) {
      return res
        .status(400)
        .json({ error: `Invalid session ID: ${sessionId}` });
    }
    ...
    for (const dist of questionDistribution) {
      ...
      if (!mongoose.Types.ObjectId.isValid(subject)) {
        return res
          .status(400)
          .json({ error: `Invalid subject ID: ${subject}` });
      }
      ...
      if (isNaN(questionCount) || questionCount <= 0) {
        return res
          .status(400)
          .json({
            error: `Invalid number of questions for subject: ${subject}`,
          });
      }
      ...
      if (questions.length < questionCount) {
        return res
          .status(400)
          .json({
            error: `Not enough questions available for subject: ${subject}`,
          });
      }
  ...
  try {
    ...
    for (const dist of exam.questionDistribution) {
      ...
      if (questions.length < questionCount) {
        return res
          .status(400)
          .json({ error: `Not enough questions for subject: ${subjectId}` });
      }
...
router.post("/submit-exam", async (req, res) => {
  try {
    ...
    Examattempted.save();
      res.json({message:'Exam Submited successfully'});
    // res.json({
    //   score,
    //   totalMarks,
    //   passed,
...
router.get("/report", async (req, res) => {
  try {
    const exams = await ExamAttempted.find()
      .populate("ExamineeId", "email name")
...
router.post("/result/:id", async (req, res) => {
  ...
  try {
    ...
    return res
      .status(500)
      .json({ message: "Server error while declaring result" });
  }
});
// get all examination
...
router.get("/examinee-result/:id", async (req, res) => {
  ...
  try {
    const examination = await ExamAttempted.findOne({
      ...
      .populate("examId")
      .populate("ExamineeId"); //examineeId
